...
def load_voyagers_content():
    ...
    # Resolve content directories relative to this module so the loader
    # works regardless of the current working directory (WSGI, gunicorn, etc.)
    base_dir = Path(__file__).resolve().parent
    content_dirs = [
        base_dir / "source" / "voyagers_vol2",
        # base_dir / "source" / "voyagers_1",  # Add Vol 1 later
    ]
...
# Attempt to load the index at import time (useful for simple dev runs).
# If the import-time load fails or the app is started by a WSGI server with a
# different working directory, a `before_first_request` hook will ensure the
# index exists before serving requests.
try:
    load_voyagers_content()
except Exception as e:
    # Don't fail import — log and defer loading until the app starts handling
    # requests (this can happen in some deployment environments).
    print(f"Initial Voyagers content load failed: {e}")


@app.before_request
def ensure_index_loaded():
    """Ensure the voyagers index is populated before serving requests.

    Use `before_request` instead of `before_first_request` for broader
    compatibility across Flask versions; the function will only load once
    because it checks whether the index is empty.
    """
    if not voyagers_index:
        print("Voyagers index empty at first request; loading now...")
        try:
            load_voyagers_content()
        except Exception as e:
            print(f"Failed to load Voyagers content on first request: {e}")
...
"""
    ...
    try:
        # Prefer Ollama if configured, else fall back to OpenAI (if available)
        assistant_message = None

        # If user explicitly enabled Ollama via env var or ollama package is present
        use_ollama = os.environ.get("USE_OLLAMA", "0") in ("1", "true", "yes")
        ollama_model = os.environ.get("OLLAMA_MODEL")

        if (use_ollama or ollama_model) and ollama is not None:
            model_name = ollama_model or os.environ.get("CHAT_MODEL", "llama3")
            # Ollama expects messages similar to OpenAI's chat API in many wrappers;
            # pass a single user message containing the system+user prompt.
            resp = ollama.chat(
                model=model_name,
                messages=[{"role": "user", "content": system_prompt + "\n\n" + user_message}],
            )

            # Ollama response shapes can vary; try common paths
            assistant_message = (
            ...
            return (
                jsonify({"error": "No LLM backend available (set USE_OLLAMA or provide OpenAI credentials)"}),
                500,
            )
